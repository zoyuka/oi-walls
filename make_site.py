...
def scanner_html(rows, by_und=None, emhist=None):
    ...
    out = ['<section id=scan><h2>Best tape today <span class=al>single names · '
           ...
           '<table><tr><th>Name</th><th title="the priced move IS implied vol, translated into the % move you are paying for">EM d/w/m</th><th>Toll</th><th>Vol</th>'
    ...
    for i, r in enumerate(rows):
        ...
        hist_ = emhist.get(lab) or []
        if r.get("emd") is not None and len(hist_) >= 5:
            hs_ = sorted(hist_)
            med_ = hs_[len(hs_) // 2]
            if med_ > 0:
                rr_ = r["emd"] / med_
                if abs(rr_ - 1) >= 0.15:
                    cl_ = "w" if rr_ > 1 else "al"
                    tape += (f'<div class=m style="font-size:10.5px"><b class={cl_}>'
                             f'{rr_:.1f}×</b> own {len(hist_)}d</div>')
        flags = []
    ...
    out.append('</table><p class=m style="font-size:11.5px;line-height:1.5">Toll = bid/ask width of the ATM straddle '
               ...
               'vs tape compares today\'s priced move with the last 5 days\' real moves; ''the <b>N×</b> line under it compares it with the same name\'s own recent mornings — a personal IV rank that deepens as the archive grows (window shown). '
...
def build(fetch):
    ...
    for sym, label in TICKERS + STOCKS:
        ...
        # flow chip retired Sep 22 audit: call-share extremes showed zero
        # next-day signal (spearman 0.00, n=26). Still archived daily for a
        # re-test at n>=100.
...
<div class=m style="font-size:11px;margin:4px 0 6px">OI = yesterday's book (OCC refresh ~6am ET) — structure, not live positioning, and no dealer-side guess is made from it. ⚡ vol&gt;OI marks strikes being rebuilt <i>today</i>. <b>First audit (Sep 22, n=45 touches):</b> touched walls held to the close no better than no-OI control levels (27% vs 25%) and showed no 1-hour stall edge — treat walls as reference geography, not support/resistance. Audit continues as the archive grows.</div><a class=btn href="OI_Walls_{label}.txt">ToS study text</a></details></section>""")
    ...
    for ssym in SCAN_ONLY:
        ...
        if row and row.get("emd") is not None and row.get("spot"):
            # minimal archive entry so each name accrues its own premium history
            arch.setdefault(row["lab"], {
                "spot": round(row["spot"], 2),
                "ems": {"day": [row["spot"], row["spot"] * row["emd"] / 100]}})
        if row:
    ...
    try:
        ...
        if act or opens:
            ...
            pan_sec = ('<div class=pansec>🟢 <b>Panic-dip signal · 30m</b> — ' + ", ".join(bits) +
                       ...
                       "not advice — first audit (Sep 22): 9–7, +0.03%/trade, on-process, kept; next grading when a real vol event tests the thesis." + rl +
    ...
    try:
        ...
        if blocks:
            ...
            ladder_sec = (
                ...
                'context: price finishes inside the day band ~3 days in 4 long-run (this past '
                'month ran hot: 61% inside — the tape out-moved its pricing); once a band breaks, '
                '~1 in 3 breaks reach the 1.5× ring and ~1 in 10 the 2× ring. <b>Per-wall hold '
                'rates failed their first audit</b> (Sep 22: touched walls held 27% vs 25% for '
                'no-OI control levels, n=45) — OI marks where interest sits, not where price '
                'stops; rank is confluence, not probability. A ladder says where '
                'a move has reasons to pause — never which way it goes.</p>')
    except Exception as e_:
        print("ladder failed:", repr(e_))
        ladder_sec = ""
    emhist = {}   # label -> [day EM% by prior morning map] — each name's own premium history
    try:
        import glob as _g3
        for f_ in sorted(_g3.glob("data/archive/*_open.json"))[-22:]:
            if f"{now_et:%Y-%m-%d}" in f_:
                continue
            try:
                tk_ = json.load(open(f_))["tickers"]
            except Exception:
                continue
            for lab_, t_ in tk_.items():
                e_ = (t_.get("ems") or {}).get("day")
                if e_ and e_[0]:
                    emhist.setdefault(lab_, []).append(100 * e_[1] / e_[0])
    except Exception:
        pass
    # ---- "Is today worth trading?" — the measured GREEN conditions, surfaced.
    # Each light cites its own backtested stat; the combination is NOT a joint
    # backtest and is never presented as one.
    greens_sec = ""
    try:
        y_clv = y_fade = False
        pxd_ = spx_daily.get("px")
        if pxd_ is not None and len(pxd_) >= 3:
            dfy = pxd_
            try:
                if dfy.index[-1].date() >= now_et.date():
                    dfy = dfy.iloc[:-1]
            except Exception:
                pass
            y_, p_ = dfy.iloc[-1], dfy.iloc[-2]
            rng_ = max(float(y_.High - y_.Low), 1e-9)
            yclv_ = (float(y_.Close) - float(y_.Low)) / rng_
            y_clv = yclv_ <= 0.10 and float(y_.Close) < float(p_.Close)
            y_gap_ = float(y_.Open) / float(p_.Close) - 1
            y_fade = (y_gap_ > 0.0025 and float(y_.Close) < float(y_.Open) and yclv_ < 0.3)
        pan_on = any(p.get("last") is not None and p["last"] < 10
                     for p in pan_state.values())
        lights = [
            ("panic dip firing", pan_on,
             "30m RSI(2)<10 on an index right now — the one timing lean that survived the "
             "167-config study (long, exit RSI>70 or ~2 days; post-haircut Sharpe ≈1.0)"),
            ("term inverted", bool(term and term.get("inv")),
             "1-day vol above 30-day: next day averaged ±1.2% vs ±0.65% normally, and "
             "straddles beat their price 34% vs 26% — the closest options get to fairly priced"),
            ("premium elevated", bool(prem_rank is not None and prem_rank > 80),
             "options pricier than 80% of days — movement is being paid for daily, and "
             "in your own ledger this was the only net-green buying regime"),
            ("fade-day lean", bool(y_fade),
             "yesterday gapped up and faded — the next session closed up 63–74% of the time "
             "(avg +0.4%), one of the few era-robust day-shape leans"),
            ("weak-close bounce", bool(y_clv and not is_opex),
             "yesterday closed at its lows — 59% next-day bounce with bigger moves "
             "(this light stays OFF into monthly OPEX, where the edge measurably vanishes)"),
        ]
        n_on = sum(1 for _, on_, _ in lights if on_)
        if n_on == 0:
            head = ("<b>No green lights.</b> Most days look like this, and they're where the "
                    "account leaked — standing aside IS the trade today. If you take a shot "
                    "anyway: at a mapped level only, half size.")
        elif n_on == 1:
            head = ("<b>One green light.</b> A shot is defensible today — at a level, "
                    "normal size, exit rules decided before entry.")
        else:
            head = (f"<b class=gon>{n_on} green lights — one of the rarer favorable days.</b> "
                    f"This is when taking the shot has actually paid. Still: at a level, "
                    f"sized to survive being wrong.")
        chips_ = "".join(
            f'<span class="glc{" on" if on_ else ""}" title="{stat_}">'
            f'{"🟢" if on_ else "⚪"} {nm_}</span>'
            for nm_, on_, stat_ in lights)
        greens_sec = (f'<div class=greens id=greens><div class=glh>Is today worth trading?</div>'
                      f'<div class=glr>{chips_}</div><div class=glt>{head} '
                      f'<span class=m>Each light is individually backtested (tap one for its '
                      f'stat); the combination is not a joint backtest — more lights is not '
                      f'multiplied odds.</span></div></div>')
    except Exception as e_:
        print("greens failed:", repr(e_))
        greens_sec = ""
...
.pansec b{{color:{TEAL}}}
.greens{{background:#121216;border:1px solid #26262b;border-radius:8px;padding:9px 11px;margin:10px 0 2px}}
.glh{{font-size:13px;font-weight:700;margin-bottom:6px}}
.glr{{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:7px}}
.glc{{font-size:11.5px;color:#6b6b73;background:#17171b;border:1px solid #222228;border-radius:20px;padding:3px 9px;white-space:nowrap}}
.glc.on{{color:#dff5ec;background:#0f2019;border-color:#2c5c48}}
.glt{{font-size:12.5px;line-height:1.55;color:#c9c9cf}}
.glt b.gon{{color:{TEAL}}}
</style></head><body>
...
<div id=posbar></div>
{"".join(cards)}
{scanner_html(scan_rows, (json.loads(my_json) or {}).get("by_und") if my_json != "null" else None, emhist)}
{big_html(bigs)}
{"".join(scards)}
